...
gdouble eval_expression(const gchar *expression) {
    GError *error = NULL;
...
void on_calculate_clicked(GtkWidget *button, gpointer data) {
    ...
    // '=' 버튼이 눌렸을 때 수학적인 계산을 수행하도록 수정
    gdouble result = strtod(expression,NULL);
...
int main(int argc, char *argv[]) {
    ...
    gtk_table_attach_defaults(GTK_TABLE(grid), button, 2, 4, 5, 6);
